...
def get_parser():
    ...
    parser.add_argument(
        '--weights',
        # default= './runs/ntu_cv_sgcn_joint-29-17640.pt',
        default=None,
...
class Processor():
    ...
    def load_model(self):
        ...
        shutil.copy2('./model/tdeform.py', self.arg.work_dir)
        print(Model)
        self.model = Model(**self.arg.model_args).cuda(output_device)
        print(self.model)
        self.loss = nn.CrossEntropyLoss().cuda(output_device)
        # self.loss = FocalLoss(self.arg.model_args['num_class'],gamma=0.5)
        if self.arg.weights:
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            self.global_step += 1
            # lr = (10**(self.global_step//100))*self.arg.base_lr  #find lr
            # for param_group in self.optimizer.param_groups:
            #     param_group['lr'] = lr
            # get data
