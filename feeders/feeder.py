...
class Feeder(Dataset):
    def __init__(self, data_path, label_path,
                 ...
                 window_size=-1, normalization=False, debug=False, use_mmap=True,argument=False):
        """
    ...
    def __getitem__(self, index):
        ...
        if self.argument:
            if V == 18:
                ...
                motion = np.concatenate((data_numpy, np.zeros((C-1, 1, V, M))), 1) - np.concatenate(
                    (np.zeros((C-1, 1, V, M)), data_numpy), 1)
                motion = np.concatenate((motion[:, 1:-1, :, :], np.zeros((C-1, 1, V, M))), 1)
                data_numpy = np.concatenate((data_numpy, motion,conf), 0)
            else:
                 motion =np.concatenate((data_numpy,np.zeros((C,1,V,M))),1) - np.concatenate((np.zeros((C,1,V,M)),data_numpy),1)
                 motion = np.concatenate((motion[:,1:-1,:,:],np.zeros((C,1,V,M))),1)
